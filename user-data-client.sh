...
######
# Install Vault Agent, Consul and Nomad
######
...
{{ with secret "pki_int/issue/nomad-cluster" "common_name=client.global.nomad" "ttl=24h" "alt_names=localhost,client.${nomad_region}.nomad" "ip_sans=127.0.0.1"}}
{{ .Data.certificate }}
{{ end }}
EONT
...
{{ with secret "pki_int/issue/nomad-cluster" "common_name=client.global.nomad" "ttl=24h"  "alt_names=localhost,client.${nomad_region}.nomad"}}
{{ .Data.issuing_ca }}
{{ end }}
EONT
